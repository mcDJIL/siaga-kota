...
import { USER_STATISTICS } from '../data/userStatistics'
  ...
  return (
    <div className="flex flex-col gap-6 p-4 sm:p-8">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        ...
        <Button variant="navy" onClick={onOpenAdd}>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <UserStatisticCard icon={Users} iconBg="bg-navy/10" iconColor="text-navy" label="Total Users" value={USER_STATISTICS.totalUsers} />
        ...
        <UserStatisticCard icon={ShieldCheck} iconBg="bg-brand-green-light" iconColor="text-brand-green-dark" label="Role Breakdown" value="">
          <div className="mt-1 flex flex-wrap gap-2">
            {USER_STATISTICS.roleBreakdown.map((role) => (
              <span key={role.code} className="rounded bg-[#E5EEFF] px-2 py-1 text-xs font-semibold tracking-[0.6px] text-text-body">
                {role.code}: {role.count}
              </span>
            ))}
          </div>
        </UserStatisticCard>
        ...
        <UserStatisticCard
          ...
          value={USER_STATISTICS.newUsersThisMonth.value}
          trendText={`${USER_STATISTICS.newUsersThisMonth.trendPercent}%`}
        />
      </div>
      ...
      <DeleteUserModal user={deleteTarget} onClose={onCloseDelete} onConfirm={onDeleteUser} />
    </div>
  )
}
