...
export function WasteCategoryDonutChart({ categories = [] }) {
  const [activeIndex, setActiveIndex] = useState(null)

  const dominant = useMemo(() => {
    if (!categories || categories.length === 0) return null
    return categories.reduce((max, item) => (item.value > max.value ? item : max), categories[0])
  }, [categories])

  const centerCategory = activeIndex === null ? dominant : categories[activeIndex]
      ...
      <div className="relative mx-auto h-64 w-full max-w-xs">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            ...
            <Legend content={() => renderLegend(categories)} verticalAlign="bottom" />
          </PieChart>
        </ResponsiveContainer>
