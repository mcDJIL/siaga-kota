        ...
        {filteredOfficers.map((officer) => (
          <li key={officer.id}>
            <button
              ...
              disabled={officer.status !== 'tersedia'}
            ...
            >
              <img src={officer.avatar} alt={officer.name} className="h-9 w-9 rounded-full object-cover" />
              <div className="flex flex-1 flex-col overflow-hidden">
                <span className="truncate text-sm font-semibold text-navy">{officer.name}</span>
                <span className="text-xs text-text-muted">
                  {officer.distance} &middot; {officer.status === 'tersedia' ? 'Tersedia' : 'Bertugas'}
                </span>
              </div>
            </button>
          </li>
